...
! 2024/09/27, GT: Added option "PAVALONIS_DUST". Set this to the ID of the dust
!    class in the input filenames (e.g. "aerosolpa70). If this is set, then
!    pixels which are identified as dust in the Pavalonis mask (i.e. have the
!    value 11), will be strongly weighted towards this type in the Bayesian
!    type selection
!
! Bugs:
...
subroutine orac_postproc(mytask, ntasks, lower_bound, upper_bound, &
    ...
    character(len=20)            :: pavalonis_dust
    logical                      :: use_ml
    ...
    ! Default value for "pavalonis_dust"
    pavalonis_dust = "__Null__"
    ...
    do while (parse_driver(11, value, label) == 0)
       ...
       case('USE_PAVALONIS_PHASE')
          if (parse_string(value, use_pavalonis_phase) /= 0) &
               call handle_parse_error(label)
       case('PAVALONIS_DUST')
          if (parse_string(value, pavalonis_dust) /= 0) &
               call handle_parse_error(label)
       case('USE_CHUNKING')
             ...
             if (use_bayesian_selection) then
                ...
                do k = 1, n_in_files
                   if (input_primary(k)%costjm(i,j) /= sreal_fill_value) then
                      ...
                      ! Note we add an offset of 1e-20 to the probability, to
                      ! prevent floating point underflow
                      a_prob = (exp(-input_primary(k)%costjm(i,j) / 2.) + 1e-20) &
                           * bayesian_weights(k)
                      if (index(in_files_primary(k), pavalonis_dust) > 1) &
                           a_prob = (a_prob + 1.0) / 2.0
