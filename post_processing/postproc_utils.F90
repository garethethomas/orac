...
subroutine copy_class_specific_inputs(i, j, indexing, primary2, primary1, &
   ...
   if (indexing%flags%do_aerosol) then
      ...
      if (indexing%NThermal .ge. 2) then
         primary2%alp(i,j)                = primary1%alp(i,j)
         primary2%alp_uncertainty(i,j)    = primary1%alp_uncertainty(i,j)
         primary2%alh(i,j)                = primary1%alh(i,j)
         primary2%alh_uncertainty(i,j)    = primary1%alh_uncertainty(i,j)
         primary2%alt(i,j)                = primary1%alt(i,j)
         primary2%alt_uncertainty(i,j)    = primary1%alt_uncertainty(i,j)
      end if
   end if
...
!     primary2%cc_total2_uncertainty(i,j) = primary1%cc_total2_uncertainty(i,j)
   ...
   if (do_secondary) then
      if (indexing%flags%do_aerosol) then
         ...
         if (indexing%NThermal .ge. 2) then
            secondary2%alp_ap(i,j)        = secondary1%alp_ap(i,j)
            secondary2%alp_fg(i,j)        = secondary1%alp_fg(i,j)
         end if
      end if
      ...
      if (indexing%flags%do_meas_error) then
         secondary2%Sy(i,j,indexing%loop_to_main_index) = secondary1%Sy(i,j,:)
      end if
      
      secondary2%y0(i,j,indexing%loop_to_main_index) = secondary1%y0(i,j,:)
